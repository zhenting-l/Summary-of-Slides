        ...
        if (useWebView) {
            AndroidView(
                ...
                update = { webView ->
                    ...
                    val htmlFile: java.io.File? = when {
                        !htmlPath.isNullOrBlank() && java.io.File(htmlPath).exists() -> {
                            java.io.File(htmlPath)
                        }
                        else -> {
                            val cleaned = MarkdownTidyUtil.tidy(markdown)
                            MarkdownHtmlTemplate.writeToCache(webView.context, cleaned, "summary-live")
                        }
                    }
                    if (htmlFile != null) {
